...
- A later retest after reconnecting the local Agent (same reference photo, `gpt-image-2`, medium, `2:3`, one image) failed with the generic workbench error `请求失败`. The generation history recorded 0 successes and 1 failure; do not click `重试` automatically. The earlier successful media URL was temporary and later returned 404, so it cannot be moved into the canvas unless the user has a saved copy.
